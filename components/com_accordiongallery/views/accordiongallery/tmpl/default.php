...
$query = "Select * FROM #__accordiongalleryc  WHERE publish = 1 " . $add_query . " ORDER BY ordnum ASC " ;
...
foreach ($cat_q_res as $curr_category) 
{
	...
	if ($ret_arr['flag']) {
		if (trim($params->get( 'default_category_id', '0' )) == $curr_category->id || ($def_cat_flag && trim($params->get( 'default_category_id', '0' )) == 0)) {
...
function write_galaccordioncp_xml_data($cat_name, $cat_id, $params)
{
	...
	$query = "Select * FROM #__accordiongallery WHERE catid = " . $cat_id . " AND publish = 1 ORDER BY ordnum ASC";
...
$xml_data .= ' <element>';
				$xml_data .= ' <link target="'.trim($params->get( 'target', '_blank' )).'"><![CDATA['.trim($curr_product->linkit).']]></link>';
...
if (stristr($_SERVER['HTTP_USER_AGENT'], 'iPhone') || stristr($_SERVER['HTTP_USER_AGENT'], 'iPad') || stristr($_SERVER['HTTP_USER_AGENT'], 'Android')) {
  ...
  <!--[if lte IE 8]>
	...
	<![endif]-->
	<div class="x_gallery" id="x_gallery" data-xml="<?php echo JURI::root() . $pic_path.'/'. $catppv_id; ?>.xml" style="width:<?php echo trim($params->get( 'gallery_width', '720' ));?>px;height:<?php echo trim($params->get( 'gallery_height', '410' ));?>px; z-index:0;">
...
?>
		...
		if(isiPhone() || isChrome()) {
			document.write('<div class="x_gallery"  data-xml="'+flashvars.xmlPath+'">	<div class="x_cover"></div><ul></ul></div>');
		}
		else {
		swfobject.embedSWF(

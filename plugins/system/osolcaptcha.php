...
class plgSystemOSOLCaptcha extends JPlugin
{
		  ...
		  $captchaVerifyURL = JURI::base()."index.php";
		   $ajax = "
						...
						window.addEvent( 'domready', function() {
								//alert($scriptToGetForm.onsubmit);
								//alert('$scriptToGetForm');
								//alert($scriptToGetForm );
								onsubmitFunction =  $scriptToGetForm.onsubmit;//$scriptToGetForm.get('onsubmit');
									...
									$scriptToGetForm.addEvent( 'submit', function(e){
										...
										//alert($('osolCaptcha-ajax-container".$index."'));
	 ...
	 function adminOptionsForm()
	   {
		   ...
		   JHTML::stylesheet('captchaStyle.css', JURI::base() . 'plugins/system/osolCaptcha/');
  ...
  <tr>
    ...
    <td valign=\"top\"  ".($vertical?"":"width=\"170\"")." >
			...
			<label>
       <a href=\"#\" onclick=\"reloadCapthcha(".$GLOBALS['totalCaptchas'].");return false;\" >".JText::_('REFRESH CAPTCHA')."</a>
    </label></td>
	".($vertical?"</tr><tr>":"")."
...
");
		...
		function getEnabledForms()
		{
			...
			//if(count($this->enabledForms)>0) return $this->enabledForms;
			...
			//echo "FFF";
			//die("<pre>".print_r($this->enabledForms,true)."</pre>");
		...
		function getNonRefererableForm($details)
		{
			...
			if(preg_match_all('@'.$formRegExp.'@isU', $body, $match_forms))
			{
				...
				{
							//echo $details['formaction_regExp'];
							...
							{								
								...
								{
									echo preg_replace("/&/","&amp;",$details['formaction_regExp']);
								}*/
								$forRegExp = $details['formaction_regExp'];//preg_quote(preg_replace("/&/","&amp;",));
								
								if(preg_match("@".$formActionRegExp."@",$match_form,$match4) &&
											  preg_match("@".$forRegExp."@",$match4[1],$match5)
											  )
								{
									//echo $forRegExp."<br />".$match4[1]."\r\n" ;
									$returnVar['js_form_access'] = array("action" => preg_replace("/&amp;/","&",$match4[1]));
									return $returnVar;
								}
							}
		...
		function shouldCheckForOSOLCaptcha()
		{
			...
			
			{
				...
				if(preg_match_all("/([^=&]+)=([^=&]+)/",$details['verifyOnVars'],$requestVar_matches))
				{
					...
					//die( "<pre>".print_r($requestVar_matches,true)."</pre>");
					...
					if($enableOnForm)
					{
						...
						else
						{
						   ...
						   else
						   {
							   ...
							   {
								   $mainframe->redirect($details['redirectOnfailure']);
								   exit;
							   }
							   //if(JRequest::getVar('task') == 'register_save')
		...
		function confirmInsertCaptcha($details)
		{
			...
			foreach($reqVars as $requestVar)
			{
			 	 preg_match_all("/([^=&]+)=([^=&]+)/",$requestVar,$requestVar_matches);
					foreach($requestVar_matches[1] as $requestVarIndex => $requestVar)
					{
						$requestVarVals = preg_split("/\|/",$requestVar_matches[2][$requestVarIndex]);
						//echo $requestVar."<pre>".print_r($requestVarVals,true)."</pre><br />";
         ...
         */
        function onAfterRender()//Dispatch()
        {
			
			$enabledForms = $this->enabledForms;//$this->getEnabledForms();
			$body = JResponse::getBody();
			
			//echo "shouldInsertOSOLCaptcha()<pre>".print_r($this->addToForms,true)."</pre>";
